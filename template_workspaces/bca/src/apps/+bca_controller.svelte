<script lang="ts">
	...
	let dashboardQuery = $state(client.invoke.bca_dashboard({ scheduled_for: dispatchDay }));
	watch(
		() => dispatchDay,
		(day) => {
			dashboardQuery = client.invoke.bca_dashboard({ scheduled_for: day });
		},
		{ lazy: true }
	);
