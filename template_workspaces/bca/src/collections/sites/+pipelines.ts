	...
	export: {
		handler: async ({ records }, api) => {
			...
			return records.map((site) => {
				...
				return {
					label: `BCA interoperability bundle · ${site.name}`,
					attachments: [
						{
							name: `bca_${code}.json`,
						...
						{
							name: `bca_${code}_job_assignments.csv`,
							contentType: 'CSV' as const,
						...
						{
							name: `bca_${code}_variations.csv`,
							contentType: 'CSV' as const,
							content: variationRows
						},
						{
							name: `bca_${code}_photo_evidence.csv`,
							contentType: 'CSV' as const,
