...
norbital_approval_id = <request id>          the row exists, is visible, is immutable
      ...
      │
      └── rejected ──►  the row is rolled back from _history to its pre-approval
...
rejected restores them from `_history`. For rows already consumed by a `PAID` run, deletion is refused
