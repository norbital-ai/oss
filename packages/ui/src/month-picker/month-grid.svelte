<script lang="ts">
	...
	import { Grid, Stack } from '#lib/layout';
...
<Stack gap="xs" role="grid" data-month-grid={year}>
	{#each [0, 4, 8] as rowStart (rowStart)}
		<Grid gap="none" tracks="repeat(4, minmax(0, 1fr))" role="row">
			{#each names.slice(rowStart, rowStart + 4) as name, offset (offset)}
