...
const collectionNavigationTargetSchema = Schema.Struct({
	collectionName: Schema.String,
	recordId: Schema.String,
	routeKey: Schema.String,
	parentRouteKey: Schema.optionalKey(Schema.String)
});
export type CollectionNavigationTarget = typeof collectionNavigationTargetSchema.Type;

export interface CollectionNavigation {
	readonly current: CollectionNavigationTarget | null;
	resolveRecordId(params: {
		collectionName: string;
		routeKey: string;
		parentRouteKey?: string;
	}): string | undefined;
	href(target: CollectionNavigationTarget): string;
	open(target: CollectionNavigationTarget): void;
	pop(): void;
...
const COLLECTION_NAVIGATION_CONTEXT = Symbol.for('@norbital-ai/ui/collection-navigation');

export function setCollectionNavigationContext(
	navigation: CollectionNavigation
): CollectionNavigation {
	setContext(COLLECTION_NAVIGATION_CONTEXT, navigation);
	return navigation;
}

export function getCollectionNavigationContext(): CollectionNavigation | undefined {
	return getContext<CollectionNavigation>(COLLECTION_NAVIGATION_CONTEXT);
}

export function resolveCollectionRecordId(
	targets: CollectionNavigationTarget | readonly CollectionNavigationTarget[] | null | undefined,
...
export function createCollectionRouteKey(params: { view: string }): string {
	return `collection:${params.view}`;
}
...
export class CollectionUrlNavigation implements CollectionNavigation {
	readonly #getUrl: () => URL;
	...
	get current(): CollectionNavigationTarget | null {
	 ...
	 *
	 * A detail registration only exists while the collection surface that owns it is mounted, and a
	 * nested surface is mounted by its parent frame's detail view. Rendering only the deepest frame
	 * therefore unmounts the very surface the deepest frame needs, so navigation renders the
	 * whole chain and keeps each ancestor alive.
	 */
	get targets(): CollectionNavigationTarget[] {
		return this.#targets();
	}
	...
	resolveRecordId(params: {
		...
		return resolveCollectionRecordId(this.#targets(), params);
	}

	#targets(): CollectionNavigationTarget[] {
	 ...
	 *
	 * Most collection surfaces use the workspace client already published above navigation. Host
	 * projections such as People are deliberately different: their rows live in a read-only local
	 * client and have no tenant collection to dispatch. The route key identifies the surface that
