	...
	const read = webReader(EffectId.make('read-source'), {
		execute: (id, request) => {
			...
			return Effect.succeed({
				output: { url: 'https://example.test/final', contentType: 'text/html', body: 'Source text' }
			});
		}
	});
...
it('refuses malformed page responses instead of extracting invented content', async () => {
	const read = webReader(EffectId.make('bad-source'), {
		execute: () => Effect.succeed({ output: { unrelated: true } })
	});
