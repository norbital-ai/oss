...
it('dumps agent and plan prompts', async () => {
	...
	try {
		const twin = cassetteTranscript(
			readCassetteFile(
				fileURLToPath(new URL('./assets/agents-skills-01.cassette.json', import.meta.url))
			)
		);
		...
		await harness.runtime.runPromise(
			agents.submit(harness.effectId('submit'), adminSubject, {
				...
				message: Agents.userAgentInput('Follow the payroll skill.'),
		...
		const req = twin.requests[0] as unknown as {
