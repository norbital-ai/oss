...
describe('hook introspection', () => {
	it('names every declared operation and phase', () => {
		expect(
			describeHooks({
				...
				delete: { prepare: handler, perRecord: { before: { handler } } }
			})
		).toEqual([
			'delete.before',
			'delete.prepare',
	...
	it('counts leaves, not files — a module declaring five handlers is five hooks', () => {
		const declaration = {
			...
			delete: { prepare: handler, perRecord: { before: { handler }, after: { handler } } }
		};
		expect(describeHooks(declaration)).toHaveLength(6);
	});
