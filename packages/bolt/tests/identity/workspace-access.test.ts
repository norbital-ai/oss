...
const addSession = (
	...
	harness.database.query(
		`with person as (insert into bolt_auth_user (id, "name", "email", "tenantId", "roles", "teams") values ($2, $2, $5, 'test-tenant', $3::jsonb, $4::jsonb) on conflict (id) do update set "roles" = excluded."roles", "teams" = excluded."teams", "email" = excluded."email", "tenantId" = excluded."tenantId" returning id) insert into bolt_auth_session (id, "token", "userId", "expiresAt") select gen_random_uuid()::text, $1, person.id, now() + interval '1 hour' from person`,
...
describe('workspace access projection', () => {
	it('reports one member per user, with their highest role and their teams', async () => {
		...
		expect(result.members.map(({ id, role }) => [id, role])).toEqual([
			['u1', 'admin'],
			['u2', 'basic']
		]);
		expect(result.members.find(({ id }) => id === 'u2')?.teams).toEqual(['People', 'Platform']);
	});
	...
	it('collapses several sessions for one user into a single member', async () => {
		...
		await harness.database.query(
			`with person as (insert into bolt_auth_user (id, "name", "email", "tenantId", "roles", "teams") values ('u1', 'u1', 'ada@example.test', 'test-tenant', '["basic"]'::jsonb, '[]'::jsonb) on conflict (id) do update set "roles" = excluded."roles", "teams" = excluded."teams", "email" = excluded."email", "tenantId" = excluded."tenantId" returning id) insert into bolt_auth_session (id, "token", "userId", "expiresAt") select gen_random_uuid()::text, 'token-second', person.id, now() + interval '2 hours' from person`
		);
		const result = await access(harness);
	...
	it('excludes another tenant, and keeps a member who is merely signed out', async () => {
		...
		await harness.database.query(
			`insert into bolt_auth_user (id, "name", "email", "tenantId", "roles", "teams") values ('u-signed-out', 'u-signed-out', 'gone@example.test', 'test-tenant', '["basic"]'::jsonb, '[]'::jsonb)`
		);
		// Another tenant's member stays out entirely, session or no session.
		await harness.database.query(
			`insert into bolt_auth_user (id, "name", "email", "tenantId", "roles", "teams") values ('u-other', 'u-other', 'other@example.test', 'other-tenant', '["admin"]'::jsonb, '[]'::jsonb)`
		);
		const result = await access(harness);
		expect(result.members.map(({ id }) => id)).toEqual(['u-signed-out', 'u1']);
	});
