...
// Template source is explicit. Realm sibling `templates/` is not a fallback.
const TEMPLATES = process.env.NORBITAL_TEMPLATE_SOURCE?.trim();
const MISSING_TEMPLATE_SOURCE =
	'No template source: set NORBITAL_TEMPLATE_SOURCE to an explicit template tree. Realm sibling templates/ is not a fallback.';

type TemplateSourceStatus = 'ready' | 'unset' | 'missing';

const templateSourceStatus = (): TemplateSourceStatus => {
	if (TEMPLATES === undefined || TEMPLATES.length === 0) return 'unset';
	return existsSync(TEMPLATES) ? 'ready' : 'missing';
};
...
describe('the schedule set, before and after the owner changed', () => {
	const sourceStatus = templateSourceStatus();
	const templates =
		sourceStatus === 'ready' && TEMPLATES !== undefined
	...
	it.skipIf(sourceStatus === 'unset')(
		'finds the crons the templates actually declare, so the diff has something to be about',
		() => {
