...
it.each(['before', 'after'] as const)(
	'retains submitted text until live rows arrive when the command finishes %s them',
	async (completionOrder) => {
		...
		try {
			...
			if (completionOrder === 'before') {
				...
				flushSync();
				expect(target.querySelector('[data-admission="pending"]')?.textContent).toContain(
			...
			if (completionOrder === 'after') finish!({ messageId: request!.submissionId! });
			await new Promise<void>((resolve) => setTimeout(resolve, 0));
			flushSync();
