...
const CompilerValues = {
	discover: (candidates: ReadonlyArray<WorkspaceDefinition>): WorkspaceDefinition | DiscoveryError => {
		if (candidates.length === 0) return new DiscoveryError({ message: 'No Bolt workspace declaration was found' });
		if (candidates.length > 1) return new DiscoveryError({ message: 'Multiple Bolt workspace declarations were found' });
		const workspace = candidates[0];
		return workspace === undefined ? new DiscoveryError({ message: 'Workspace discovery failed' }) : workspace;
	},
	generateTypes: (workspace: WorkspaceDefinition): string => {
		...
		return ['// Generated by Bolt. Do not edit.', `export type CollectionName = ${collectionNames.length === 0 ? 'never' : collectionNames.join(' | ')};`, `export type AppName = ${appNames.length === 0 ? 'never' : appNames.join(' | ')};`, ''].join('\n');
	}
};
...
export type Interface = Readonly<{
	readonly discover: (candidates: ReadonlyArray<WorkspaceDefinition>) => Effect.Effect<WorkspaceDefinition, DiscoveryError>;
	readonly compile: (workspace: WorkspaceDefinition, input: ManifestInput) => Effect.Effect<Compilation, CompilationError>;
