...
/**
 * A conversation the subject took part in: a message in it carries their id as its author.
 *
 * An envoy chat is one conversation per transport chat, owned by whoever opened it; every other
 * sender in a group reaches it only through their own messages, so authorship is the only durable
 * record of who was there.
 */
// ponytail: a registered group member who never posted is invisible; add transport membership
// when the transport reports it.
const PARTICIPANT_TASK = Object.freeze({
	messages: {
		some: { author: { jsonPath: { path: ['id'], type: 'string', eq: SUBJECT_ID } } }
	}
});
type EnvoyReach = Readonly<{ readonly name: string; readonly audience: string }>;
/** Readable: `context_window_tokens` is what compaction was judged against; `capability_snapshot` stays unexposed. */
const TURN_READ_FIELDS = [
	'id',
	'conversation_id',
	'input_message_id',
	'mode',
...
/** The grants that open one set of conversations and everything hanging off them. */
export const CONVERSATION_COLLECTIONS: ReadonlySet<string> = new Set([
	collections.conversation.name,
	collections.plan.name,
	collections.conversation_message.name,
	collections.turn.name,
	collections.turn_usage.name
]);
const conversationGrants = (
	task: Readonly<Record<string, unknown>>
): NonNullable<PolicyDeclaration['grants']> => [
	{ collection: collections.conversation.name, action: 'read' as const, where: task },
	...[collections.plan, collections.conversation_message].map((systemCollection) => ({
		collection: systemCollection.name,
		action: 'read' as const,
		where: { task: { some: task } }
	})),
	{
		collection: collections.turn.name,
		action: 'read' as const,
		where: { task: { some: task } },
		fields: TURN_READ_FIELDS
	},
	{
		collection: collections.turn_usage.name,
		action: 'read' as const,
		where: { run: { some: { task: { some: task } } } }
	}
];
/**
 * What a member may read: their own conversations, conversations they took part in, and every
 * conversation of a `public` envoy — a chat anyone could have walked into is not private to the
 * member who happened to open it. An `authenticated` envoy's chats stay with their participants.
 */
const readableTask = (envoys: ReadonlyArray<EnvoyReach>): Readonly<Record<string, unknown>> => {
	const publicEnvoys = envoys
	...
	return Object.freeze({
		OR: [
			OWN_TASK,
			PARTICIPANT_TASK,
			...(publicEnvoys.length === 0
				? []
				: [Object.freeze({ ...WORKBENCH_TASK, agent_id: { in: publicEnvoys } })])
		]
	});
};
...
export const systemReadPolicy = (envoys: ReadonlyArray<EnvoyReach>): PolicyDeclaration =>
	Object.freeze<PolicyDeclaration>({
		...
		grants: [
			...
			...conversationGrants(readableTask(envoys)),
			{
...
/** The envoy-less shape, for callers and tests that ask what the policy grants in the abstract. */
export const SYSTEM_READ_POLICY: PolicyDeclaration = systemReadPolicy([]);
...
export const withSystemCollections = <
	T extends {
		...
		readonly envoys?: ReadonlyArray<EnvoyReach>;
	}
>(
	definition: T
): T => {
	...
	const absent = builtInPolicies(definition.envoys ?? []).filter(({ name }) => !declared.has(name));
