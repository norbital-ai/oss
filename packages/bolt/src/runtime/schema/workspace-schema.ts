...
import { SYSTEM_COLUMN_NAMES, buildSchemaPlan, planTableNames, type SchemaPlan } from '../../compiler/schema-plan.js';
...
export interface Interface {
	...
	readonly migrate: (effectId: EffectId) => Effect.Effect<void, Database.FacilityError | SchemaValidationError>;
	/** The divergences between the authored collections and the live tables; empty means verified. */
	readonly verify: (effectId: EffectId) => Effect.Effect<ReadonlyArray<string>, Database.FacilityError>;
...
export const layer = Layer.effect(Service, Effect.gen(function* () {
	...
	const verify = Effect.fn('WorkspaceSchema.verify')(function* (effectId: EffectId) {
		...
		return [
			...planOnlyTables.filter((name) => !live.has(name)).map((name) => `${name}: table is missing`),
			...[...declaredColumns]
				...
				.flatMap(([name, declared]) => {
					...
					return [
						...(absent.length === 0 ? [] : [`${name}: missing column${absent.length === 1 ? '' : 's'} ${absent.join(', ')}`]),
						...(unexpected.length === 0 ? [] : [`${name}: unexpected column${unexpected.length === 1 ? '' : 's'} ${unexpected.join(', ')}`])
					];
				})
		];
	});
	const validate = Effect.fn('WorkspaceSchema.validate')(function* () {
		...
		for (const step of schemaPlan.steps) {
			if (step.id.trim() === '' || step.sql.trim() === '') return yield* new SchemaValidationError({ message: 'Schema plan contains an empty migration step' });
			if (ids.has(step.id)) return yield* new SchemaValidationError({ message: `Schema plan contains duplicate step ${step.id}` });
			ids.add(step.id);
		}
	});
	/**
	 ...
	 */
	const lineage = [...(workspace.definition.migrations ?? [])].toSorted((left, right) => left.tag.localeCompare(right.tag));
	/**
	 ...
	 */
	const baselineLineage = Effect.fn('WorkspaceSchema.baselineLineage')(function* (effectId: EffectId) {
	...
	return Service.of({
		...
		migrate: Effect.fn('WorkspaceSchema.migrate')(function* (effectId) {
			...
			const readField = (row: Schema.Json | undefined, key: string): unknown =>
				typeof row === 'object' && row !== null && !Array.isArray(row) ? Reflect.get(row, key) : undefined;
			const planned = readField(stateTable.rows[0], 'present') === true
				? Number(
						readField(
							(yield* database.execute(effectId, {
								_tag: 'Query',
