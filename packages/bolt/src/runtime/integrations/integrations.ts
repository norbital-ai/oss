...
import { runOutboxDrain, DRAIN_BATCH_DEFAULT, type ClaimedDelivery, type DeliverDependencies } from './deliver.js';
 ...
 * nothing to count.
 */
export const IntegrationStatus = Schema.Struct({ name: Schema.NonEmptyString, enabled: Schema.Boolean, cursor: Schema.Json, pending: Schema.Number, failed: Schema.Number });
export interface IntegrationStatus extends Schema.Schema.Type<typeof IntegrationStatus> {}
export type Interface = Readonly<{
	readonly install: (effectId: EffectId, name: string) => Effect.Effect<void, IntegrationError | Database.FacilityError>;
	/**
	 ...
	 */
	readonly pull: (effectId: EffectId, name: string, cursor: Schema.Json, binding?: string) => Effect.Effect<Schema.Json, IntegrationError | Database.FacilityError>;
	/**
	...
	readonly flush: (effectId: EffectId, name: string, input: Schema.Json) => Effect.Effect<Schema.Json, IntegrationError | Database.FacilityError>;
	readonly reconcile: (effectId: EffectId, name: string) => Effect.Effect<void, IntegrationError | Database.FacilityError>;
	readonly disable: (effectId: EffectId, name: string) => Effect.Effect<void, IntegrationError | Database.FacilityError>;
	readonly status: (effectId: EffectId, name: string) => Effect.Effect<IntegrationStatus, IntegrationError | Database.FacilityError>;
}>;
...
const claimedDeliveries = (rows: ReadonlyArray<Schema.Json>): ReadonlyArray<ClaimedDelivery> =>
	rows.flatMap((row) => {
		...
		if (!Number.isFinite(sequence) || typeof binding !== 'string' || typeof collection !== 'string' || typeof recordId !== 'string' || typeof operation !== 'string') return [];
		return [{
			sequence,
			binding,
			collection,
...
const cursorsOf = (value: Schema.Json): Readonly<Record<string, string>> =>
	value === null || typeof value !== 'object' || Array.isArray(value)
		? {}
		: Object.fromEntries(
				Object.entries(value).flatMap(([binding, cursor]) => (typeof cursor === 'string' ? [[binding, cursor] as const] : []))
			);
...
export const layer = Layer.effect(Service, Effect.gen(function* () {
	...
	const requireIntegration = Effect.fn('Integrations.require')(function* (name: string) {
		const integration = workspace.definition.integrations.find((candidate) => candidate.name === name);
		if (integration === undefined) return yield* new IntegrationError({ integration: name, message: 'Unknown integration' });
		return integration;
	});
	...
	const dependencies = (integrationName: string, subject: Identity.Subject): PullDependencies => ({
			...
			connector.execute(effectId, {
				...
				Effect.flatMap((response) =>
					Schema.decodeUnknownEffect(IntegrationHttpResponse)(response.output).pipe(
						Effect.mapError((issue) => ({ message: `connector answered something that is not an HTTP response: ${describeCause(issue)}`, retryable: false }))
					)
				),
		...
		secret: (effectId, name) =>
			secrets.read(effectId, name).pipe(
				...
				Effect.flatMap((value) =>
					value === null
						? Effect.fail({ message: `${integrationName} needs the environment variable ${name}, and the vault has no value for it` })
		...
		pipeline: (effectId, collection, record) => {
			...
			const api = makeAuthoringApi(makeBoundAuthoringOps(effectId, subject, collections, ai, files));
			return Effect.suspend(() =>
				runAuthoredHandler(declared.handler({ input: [record], api }, api))
			).pipe(
				Effect.flatMap((rows) =>
					Array.isArray(rows)
						? Effect.succeed(rows.flatMap((row) => (row !== null && typeof row === 'object' && !Array.isArray(row) ? [row as Readonly<Record<string, unknown>>] : [])))
						: Effect.fail({ message: `the import pipeline for ${collection} returned something that is not a list of rows` })
				),
		...
		resolve: (effectId, run) => {
			...
			return Effect.suspend(() => runAuthoredHandler(run(api))).pipe(
				Effect.catchCause((cause) => Effect.fail({ message: `${integrationName} failed to resolve a batch: ${describeCause(cause)}` }))
			);
		},
	 ...
	 */
	const webhookDependencies = (integrationName: string, subject: Identity.Subject): WebhookDependencies => {
		...
		return {
						...
						inserted.rows.length === 1
							...
							: database.execute(effectId, {
									...
									Effect.map((existing) => {
										...
										const status = row === null || row === undefined || typeof row !== 'object' || Array.isArray(row) ? undefined : Reflect.get(row, 'status');
			...
			settle: (effectId, entry) =>
				...
				}).pipe(Effect.asVoid, Effect.mapError((error) => ({ message: describeCause(error) })))
		};
	};
		...
		return {
			...
			claim: (effectId, integrationName_, limit) =>
				database.execute(effectId, {
					...
					Effect.map((result) => claimedDeliveries(result.rows).toSorted((left, right) => left.sequence - right.sequence)),
					Effect.mapError((error) => ({ message: describeCause(error) }))
				),
					...
					settlement._tag === 'Delivered'
						...
						: settlement._tag === 'Retry'
							? {
									...
									parameters: [settlement.sequence, settlement.status, settlement.reason, settlement.delayMs]
								}
							: {
									_tag: 'Query',
	 ...
	 */
	const claimPull = Effect.fn('Integrations.claimPull')(function* (effectId: EffectId, name: string) {
		...
		const value = row === null || typeof row !== 'object' || Array.isArray(row) ? null : Reflect.get(row, 'cursor');
	 ...
	 */
	const releasePull = (effectId: EffectId, name: string, cursors: Readonly<Record<string, string>>) =>
		database.execute(effectId, {
			_tag: 'Query',
	...
	return Service.of({
		install: Effect.fn('Integrations.install')(function* (effectId, name) {
			...
			yield* database.execute(effectId, { _tag: 'Query', sql: 'insert into bolt_integrations (name, enabled, cursor) values ($1, true, $2) on conflict (name) do update set enabled = true', parameters: [name, null] });
			yield* tasks.execute(effectId, { _tag: 'Enqueue', command: 'integrations.pull', input: { name, cursor: null } });
		}),
		/**
		...
		pull: Effect.fn('Integrations.pull')(function* (effectId, name, cursor, binding) {
			...
			if (module === undefined) {
				return yield* new IntegrationError({ integration: name, message: `${name} is declared but its authored module did not reach the runtime` });
			}
			...
			if (selected.length === 0) {
				return yield* new IntegrationError({ integration: name, message: `${name} declares no receive binding named ${String(binding)}` });
			}
			...
			for (const declared of selected) {
				...
				if (authoredBinding === undefined) {
					failures.push({ binding: declared.name, reason: 'the authored binding did not reach the runtime' });
					continue;
				}
				const from = overrides[declared.name] ?? stored[declared.name] ?? null;
				const outcome = yield* Effect.result(runPullBinding(bound, effectId, integration, declared, authoredBinding, from));
			...
			for (const report of reports) if (report.cursor !== null) cursors[report.binding] = report.cursor;
			...
			return {
				...
				reason: null,
				bindings: reports.map((report) => ({ ...report, rejected: report.rejected.map(({ index, reason }) => ({ index, reason })) })),
				failures
			};
		}),
		/**
		...
		receive: Effect.fn('Integrations.receive')(function* (effectId, name, bindingName, delivery) {
			...
			if (binding === undefined) {
				...
				return yield* new IntegrationError({ integration: name, message: `${name} declares no webhook binding named ${bindingName}` });
			}
			const module: AuthoredIntegrationModule | undefined = authored.integrations[name];
			const authoredBinding = module?.receive[bindingName];
			if (authoredBinding === undefined) {
				return yield* new IntegrationError({ integration: name, message: `${name}.${bindingName} is declared but its authored binding did not reach the runtime` });
			}
			const subject = integrationSubject(name);
			const report = yield* runWebhookDelivery(
				...
				authoredBinding,
				delivery
			).pipe(Effect.mapError((error) => new IntegrationError({ integration: name, message: error.message })));
		...
		flush: Effect.fn('Integrations.flush')(function* (effectId, name, input) {
			...
			if (integration.send.length === 0) {
				...
				return yield* new IntegrationError({ integration: name, message: `${name} declares no send binding, so it has no outbox to flush.` });
			}
			const limit = input !== null && typeof input === 'object' && !Array.isArray(input) && typeof Reflect.get(input, 'limit') === 'number'
				? Number(Reflect.get(input, 'limit'))
				: DRAIN_BATCH_DEFAULT;
			const report = yield* runOutboxDrain(deliverDependencies(name), effectId, integration, limit).pipe(
				Effect.mapError((error) => new IntegrationError({ integration: name, message: error.message }))
			);
			return {
		...
		reconcile: Effect.fn('Integrations.reconcile')(function* (effectId, name) {
			...
			yield* database.execute(effectId, { _tag: 'Query', sql: 'update bolt_integrations set cursor = null where name = $1', parameters: [name] });
			yield* tasks.execute(effectId, { _tag: 'Enqueue', command: 'integrations.pull', input: { name, cursor: null } });
		}),
		disable: Effect.fn('Integrations.disable')(function* (effectId, name) {
			yield* requireIntegration(name);
			yield* database.execute(effectId, { _tag: 'Query', sql: 'update bolt_integrations set enabled = false where name = $1', parameters: [name] });
		}),
		status: Effect.fn('Integrations.status')(function* (effectId, name) {
			yield* requireIntegration(name);
			const result = yield* database.execute(effectId, { _tag: 'Query', sql: 'select enabled, cursor from bolt_integrations where name = $1', parameters: [name] });
			const row = result.rows[0];
			const record = row === null || row === undefined || typeof row !== 'object' || Array.isArray(row) ? undefined : row;
			...
			const counted = (key: string): number => {
				const value = counts === null || counts === undefined || typeof counts !== 'object' || Array.isArray(counts) ? undefined : Reflect.get(counts, key);
