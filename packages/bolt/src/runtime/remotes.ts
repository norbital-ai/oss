...
import { makeAuthoringApi, makeBoundAuthoringOps, runAuthoredHandler } from './collections/authored.js';

export type RuntimeRemoteApi = Readonly<{
	readonly db: object;
	readonly infer: (input: { readonly schema: Schema.Codec<unknown, unknown>; readonly prompt: string; readonly model?: string }) => Promise<unknown>;
	readonly readFileAsset: (assetId: string) => Promise<{ readonly id: string; readonly name: string; readonly mimeType: string | null; readonly size: number; readonly bytes: Uint8Array }>;
}>;

export type RuntimeRemoteHandler = ReturnType<() => (input: unknown, api: RuntimeRemoteApi) => Promise<unknown>>;
...
type RuntimeRemoteRegistry = Readonly<{
	readonly invoke: (name: string, input: unknown, subject: Identity.Subject, effectId: EffectId) => Effect.Effect<Schema.Json, DispatchError>;
}>;

/** Resolves authored remote handlers through the same collection, AI, and file capabilities used by native runtime operations. */
export const RemoteRegistry = Context.Service<RuntimeRemoteRegistry>('@norbital-ai/bolt/RemoteRegistry');
...
const RemoteInputs = {
	decode: (input: unknown): Promise<Readonly<Record<string, Schema.Json>>> => Schema.decodeUnknownPromise(RemoteValues)(input ?? {}),
	/**
...
type QueryOptions = Readonly<{ readonly limit?: number; readonly where?: unknown; readonly orderBy?: unknown }>;
...
layer: (handlers: Readonly<Record<string, RuntimeRemoteHandler>>) => Layer.effect(
	RemoteRegistry,
	Effect.gen(function* () {
		...
		return RemoteRegistry.of({
			invoke: Effect.fn('RemoteRegistry.invoke')(function* (name, input, subject, effectId) {
				...
				if (handler === undefined) return yield* new DispatchError({ code: 'unknown_remote', message: `Unknown workspace remote: ${name}` });
				...
				return yield* Schema.decodeUnknownEffect(Schema.Json)(output).pipe(
					Effect.mapError(() => new DispatchError({ code: 'invalid_remote_output', message: `Remote ${name} returned a non-JSON value` }))
				);
			})
		});
	})
