...
export class AuthenticationError extends Schema.TaggedError<AuthenticationError>()(
	'Bolt.Identity.AuthenticationError',
	{
		reason: Schema.Literals([
			'invalid', 'expired', 'revoked', 'malformed'
		])
	}
) {
...
export type Interface = Readonly<{
	...
	readonly startSession: (effectId: EffectId, userId: string, tenantId: string) => Effect.Effect<string, AuthenticationError | Database.FacilityError>;
	readonly endSession: (effectId: EffectId, credential: string) => Effect.Effect<void, Database.FacilityError>;
	readonly workspaceSettings: (effectId: EffectId, tenantId: string) => Effect.Effect<Schema.Json, Database.FacilityError>;
	/** Who belongs to the workspace, what is outstanding, and what has changed. */
	readonly workspaceAccess: (effectId: EffectId, tenantId: string) => Effect.Effect<WorkspaceAccess, Database.FacilityError>;
}>;

/** The workspace-access projection the settings surfaces render. */
export type WorkspaceAccess = Readonly<{
	readonly members: ReadonlyArray<Readonly<{ id: string; email: string; name: string; role: string; status: string; teams: ReadonlyArray<string> }>>;
	readonly invitations: ReadonlyArray<Readonly<{ id: string; email: string; role: string; status: string; invitedBy?: string; expiresAt?: string }>>;
	readonly teams: ReadonlyArray<Readonly<{ id: string; name: string }>>;
	readonly events: ReadonlyArray<Readonly<{ id: string; action: string; actor: string; subject?: string; at: string }>>;
}>;
 ...
 */
export const layerWith = (canDeliver: boolean) => Layer.effect(
	Service,
	Effect.gen(function* () {
		const database = yield* Database.Service;
		const communication = yield* Communication.Service;
		const identityHooks = yield* IdentityHooks.Service;
		const readSubject = Effect.fn('Identity.readSubject')(function* (effectId: EffectId, sql: string, parameters: ReadonlyArray<Schema.Json>) {
		...
		const authFor = Effect.fn('Identity.authFor')(function* (effectId: EffectId) {
			...
			return makeAuth({
				...
				execute: (sql, parameters) =>
					Effect.runPromise(
						database
							.execute(effectId, { _tag: 'Query', sql, parameters: parameters as ReadonlyArray<Schema.Json> })
							.pipe(Effect.map((result) => ({ rows: result.rows as ReadonlyArray<Record<string, unknown>>, affectedRows: result.affectedRows })))
					),
				deliver: (message) =>
					Effect.runPromise(
						communication
		...
		return Service.of({
			...
			invite: Effect.fn('Identity.invite')(function* (effectId, tenantId, email, invitedBy) {
				...
				yield* database.execute(effectId, { _tag: 'Query', sql: 'insert into bolt_invitations (invitation_id, tenant_id, email, invited_by, status) values ($1, $2, $3, $4, $5) on conflict (invitation_id) do nothing', parameters: [invitationId, tenantId, email, invitedBy, 'pending'] });
				yield* communication.execute(effectId, { _tag: 'Notify', recipient: email, payload: { kind: 'workspace_invitation', invitationId, tenantId } });
				yield* identityHooks.emit(effectId, { _tag: 'UserInvited', invitationId, organizationId: tenantId, email, invitedBy });
				return invitationId;
			}),
			acceptInvitation: Effect.fn('Identity.acceptInvitation')(function* (effectId, invitationId, userId) {
				const result = yield* database.execute(effectId, { _tag: 'Query', sql: 'update bolt_invitations set status = $2, accepted_by = $3 where invitation_id = $1 and status = $4 returning invitation_id, tenant_id, email', parameters: [invitationId, 'accepted', userId, 'pending'] });
				...
				if (organizationId === undefined) return yield* new AuthenticationError({ reason: 'malformed' });
				const email = IdentityRows.text(row, 'email');
			...
			endSession: Effect.fn('Identity.endSession')(function* (effectId, credential) {
				...
				yield* database.execute(effectId, { _tag: 'Query', sql: `delete from ${AUTH_MODELS.session} where "token" = $1`, parameters: [credential] });
			}),
			/**
			 ...
			 */
			workspaceAccess: Effect.fn('Identity.workspaceAccess')(function* (effectId: EffectId, tenantId: string) {
				...
				const members = memberRows.rows.map((row) => {
					...
					return {
						...
						name: IdentityRows.text(row, 'email')?.split('@')[0] ?? IdentityRows.text(row, 'id') ?? '',
						role: roles.includes('admin') ? 'admin' : roles.includes('manager') ? 'manager' : 'basic',
						status: 'active',
						teams: IdentityRows.strings(row, 'teams')
					};
				});
				...
				return {
					...
					events: auditRows.rows.map((row) => {
						...
						const subject = Schema.is(Schema.Record(Schema.String, Schema.Json))(payload)
							? IdentityRows.text(payload, 'collection') ?? IdentityRows.text(payload, 'requestId')
							: undefined;
						return {
							id: IdentityRows.text(row, 'id') ?? 'unknown',
			...
			workspaceSettings: Effect.fn('Identity.workspaceSettings')(function* (effectId, tenantId) {
				const result = yield* database.execute(effectId, { _tag: 'Query', sql: 'select settings from bolt_workspace_identity_settings where tenant_id = $1', parameters: [tenantId] });
				const row = result.rows[0];
				const settings = typeof row === 'object' && row !== null ? Reflect.get(row, 'settings') : undefined;
