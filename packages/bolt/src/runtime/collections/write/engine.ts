...
export type GraphPreparePorts<Error = unknown, Requirements = never> = Readonly<{
	...
	readonly stageHookWrites: HookWriteOps;
...
export const makeGraphPreparers = <Error, Requirements>(
	...
	const prepareDelete: GraphPrepareFns<Error | AuthoredRefusal, Requirements>['prepareDelete'] = Effect.fn(
		'Collections.prepareGraphDelete'
	...
	const prepareNode: GraphPrepareFns<Error | AuthoredRefusal, Requirements>['prepareNode'] = Effect.fn('Collections.prepareGraphNode')(
		...
		function* (
			...
			const id = identity?.id ?? (typeof submittedId === 'string' ? submittedId : ports.randomId());
