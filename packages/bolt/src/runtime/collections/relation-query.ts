...
/**
 * A `with` clause, as a Drizzle relational query.
 *
...
export type LevelPlan = Readonly<{
...
/** The ordering a read applies, as the expressions Drizzle's `orderBy` callback returns. */
export const orderingExpressions = (
	...
	return terms.flatMap((term) => {
		...
		const expression = term.collation === undefined ? sql`${column}` : sql`${column} collate "C"`;
...
const levelWhere = (
	...
	return Result.succeed(and(visibility, whereExpression(compiled.success)) ?? visibility);
};
