	...
	Effect.gen(function* () {
		...
		return Service.of({
			...
			resume: Effect.fn('Automations.resume')(function* (effectId, taskId, input) {
				...
				yield* database.execute(effectId, {
					_tag: 'Query',
					sql: 'update bolt_automation_runs set state = $2 where task_id = $1',
					parameters: [taskId, 'resumed']
				});
			}),
			status: Effect.fn('Automations.status')(function* (effectId, taskId) {
				return (yield* database.execute(effectId, {
					_tag: 'Query',
					sql: 'select state from bolt_automation_runs where task_id = $1',
					parameters: [taskId]
				})).rows[0];
			}),
