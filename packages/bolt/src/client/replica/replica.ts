...
export type OptimisticMutation = Readonly<{
	readonly id: string;
...
const ReplicaValues = {
	settleOptimistic: (mutation: OptimisticMutation, accepted: boolean): OptimisticMutation => ({
		...mutation,
...
export type ReplicaStatus = Readonly<{
...
export class ReplicaError extends Schema.TaggedError<ReplicaError>()('Bolt.Replica.Error', {
...
export type Interface = Readonly<{
	...
	readonly query: (
		sql: string,
		parameters: ReadonlyArray<Schema.Json>
	) => Effect.Effect<ReadonlyArray<Schema.Json>, ReplicaError>;
...
export const layer = (sql: LocalSql) =>
	Layer.effect(
		Service,
		Effect.gen(function* () {
			const status = yield* Ref.make<ReplicaStatus>({
			...
			return Service.of({
						...
						yield* Effect.tryPromise({
							try: () => sql.applyChange(change),
								...
								new ReplicaError({
									...
									message: 'Failed to apply a replica diff',
									cause
								})
						});
				...
				query: Effect.fn('Replica.query')((query, parameters) =>
					Effect.tryPromise({
						try: () => sql.query(query, parameters),
						catch: (cause) =>
							new ReplicaError({
								operation: 'query',
								message: 'Failed to query the local replica',
								cause
							})
					})
				),
				reset: Effect.fn('Replica.reset')(function* () {
					yield* Effect.tryPromise({
						try: () => sql.reset(),
							...
							new ReplicaError({
								operation: 'reset',
								message: 'Failed to reset the local replica',
								cause
							})
					});
