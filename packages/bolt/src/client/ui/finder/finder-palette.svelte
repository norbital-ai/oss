			...
			{#snippet itemSnippet({ item, isIndicator })}
				...
				{#if row}
					...
					<Inline
						...
						class="{row.kind === 'group' ? 'text-overline px-3' : 'px-3'} {highlighted
					...
					>
						...
						-->
							<span
								class="truncate text-xs font-normal text-foreground sm:text-micro {row.description
									? 'w-40 shrink-0 sm:w-52'
									: 'min-w-0 flex-1'}">{row.label}</span
							>
							{#if row.description}
