...
export class PodSyncClient {
	...
	/** Limit server-side diff resolution to collections this replica actually materializes. */
	setSubscribedCollections(collections: Iterable<string>): void {
		...
		if (this.streamLoop) {
			void this.stopStream().then(() => this.startStream());
		}
	}
		...
		while (!signal.aborted) {
			try {
				...
				const collectionsParam = encodeURIComponent(
					[...this.subscribedCollections].sort().join(',')
				);
				const response = await this.httpFetch(
					`sync/stream?cursor=${cursorParam}&collections=${collectionsParam}`,
					{
						method: 'GET',
