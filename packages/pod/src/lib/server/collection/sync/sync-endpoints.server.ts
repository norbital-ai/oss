				...
				while (!signal.aborted) {
					...
					if (batch.rows.length > 0) {
						...
						for (let start = 0; start < diffs.length; start += STREAM_EMIT_CHUNK_SIZE) {
							...
							// The hosted runtime multiplexes every response over one framed stdout
							// pipe. A backlog must yield between bounded frames or its always-ready
							// SSE reader can delay a mutation/download response indefinitely.
