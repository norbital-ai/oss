...
remain visible. Each overlap detector is an ordinary `packs/overlaps/*.yaml` rule, and every
documented bad and good example is executed by `tests/examples-execute.test.ts`.
