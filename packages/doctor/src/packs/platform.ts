...
const INFRASTRUCTURE =
	/(?:^|\/)(?:migrations?|compiler|generator|schema|drizzle)(?:\/|$)/i;
...
function transactionStatementOwner(
	...
	const isTransactionObject = (candidate: import('typescript').ObjectLiteralExpression): boolean =>
		candidate.properties.some((property) => {
			...
			return ts.isStringLiteral(property.initializer) && property.initializer.text === 'Transaction';
		});
...
const rawSql = defineRule({
	...
	check(node, context) {
		...
		// `$sql` is the policy compiler's deliberately narrow relation-predicate input. It is stored
		// as policy metadata and compiled into bootstrap policy DDL; an ordinary `sql` property is
		// not this exception.
		if (
			policyPredicate ||
...
function generatedQueryBinding(name: string, context: import('../rules.js').RuleContext): boolean {
	const ts = context.ts;
	let found = false;
...
function syncSseOwner(context: import('../rules.js').RuleContext): boolean {
	return (
		/(?:^|\/)api\/bolt\/sync\/stream(?:\/|$)/.test(context.file) ||
		/(?:^|\/)client\/replica\/subscribe\.[cm]?[jt]s$/.test(context.file)
	);
}
...
const nonSyncSse = defineRule({
	...
	check(node, context) {
		...
		context.report(node, `transport=${node.text === 'sse' ? 'sse' : 'text/event-stream'} owner=non-sync`);
	}
});
...
const rawTransportCommand = defineRule({
	...
	when: ['CallExpression'],
	// The summary says client UI, and the rule now means it. Eleven of its thirteen findings in the
	// realm were the transport implementations themselves — bolt's replica bootstrap and sync wake,
	// Colony's Node socket server and its CDP driver — which is the layer that has to call `send`.
	// A rule about callers of a layer does not describe the layer itself.
	files: ['**/ui/**', '**/*.svelte'],
	check(node, context) {
		const callee = context.calleeName(node) ?? '';
