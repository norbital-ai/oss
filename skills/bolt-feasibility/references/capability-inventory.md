...
| Structured domain values (money, bank account, work pattern, statutory regime, accrual keys, eligibility rules, payroll snapshots) | `src/custom-types/<name>/` (+definition.ts + +renderer.svelte) | hr-payroll (money, bank_account, work_pattern, …) |
| Effective-dated facts (employee facts that change over time)                                                                       | collections + hooks                                            | hr-payroll                                        |
| System columns on every row: creator, timestamps, history                                                                          | runtime                                                        | `norbital-platform` skill                         |
| Temporal history, rollback, audit trail                                                                                            | runtime                                                        | `norbital-platform` skill                         |
...
| Permission grants per team, write-then-lock approvals, approval routing, audit of who approved                     | `src/policies/+<name>.policy.ts` + `src/+teams.ts` | field-operations contractor/controller/WhatsApp policies, hr-payroll |
...
| Expression language for computed/validated values                                                                  | `@norbital-ai/std` cel           | hr-payroll                                                           |
| Money arithmetic: currency, rounding, tax modes, snapshots                                                         | `@norbital-ai/std` finance       | crm (net/tax/line_total), all `money` custom types                   |

## People, communication, and AI

| Capability                                                                                                              | Where it lives                      | Precedent                                               |
| ----------------------------------------------------------------------------------------------------------------------- | ----------------------------------- | ------------------------------------------------------- |
| Chat channels with external, accountless users (WhatsApp), scoped to their own records                                  | `src/channels/+<name>.channel.ts`   | field-operations contractor WhatsApp                    |
| Interactive workspace agent, per-workspace model, plan mode, verifier                                                   | `src/+agent.ts`                     | all templates                                           |
| Custom agent tools                                                                                                      | `src/tools/+<name>.tool.ts`         | —                                                       |
| Workspace agent skills                                                                                                  | `.agents/skills/<name>/SKILL.md`    | —                                                       |
| Remote MCP servers, allowlisted per tool                                                                                | `src/mcp/+<name>.mcp.ts`            | —                                                       |
