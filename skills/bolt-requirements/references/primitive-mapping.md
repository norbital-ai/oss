...
| A structured value that is not a row (bank account, work pattern, regime) | a `defineCustomType` definition + renderer, accessed with `custom('<name>')` | JSONB storage; tenant definitions are filesystem-discovered, while platform definitions such as `money` and `instant_range` are injected |
...
| Requirement                                                                   | Construction                                                                                  | Notes                                                                                   |
| ----------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| Money anywhere                                                                | `custom('money', { allowedCurrencies })` (platform-injected datatype)                                 | currency, rounding, minor units from `@norbital-ai/std` finance                         |
...
| Requirement                                | Construction                                              | Notes                                                                                                                                                                                           |
| ------------------------------------------ | --------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Internal people with different permissions | `src/access/policies/+<name>.ts` + `src/access/+teams.ts` | policy is source: who may read what, what needs approval. A person is on exactly one team, so a combination of authority is its own named team; there are no roles. Precedent: field-operations |
| A decision that needs a second person      | an approval flow on the policy                            | prepare-and-gate: proposed values remain uncommitted while pending; approval resumes commit; the `norbital-platform` skill has the behaviour                                                    |
